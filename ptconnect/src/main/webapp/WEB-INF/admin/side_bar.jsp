...
function activeNavItem(){
	...
	var navIndex = "${menu_location}";
	if(navIndex !== undefined){
		var navLoc = navIndex.split(',');
		//사이드바  선택된 메뉴에 활성화 & 강조표시(위에서부터(0부터) n번째)
		$(currentNav[navLoc[0]]).addClass("active");
		$(currentNav[navLoc[0]]).find('.collapse').addClass('show');
		if(navLoc[1]!="0"){
			var c_item_index = navLoc[1] - 1;
			let currentCollapseItem = $(currentNav[navLoc[0]]).find('.collapse-item');
			console.log(currentCollapseItem[c_item_index]);
			$(currentCollapseItem[c_item_index]).addClass('active');
		}
	}
}
activeNavItem();
